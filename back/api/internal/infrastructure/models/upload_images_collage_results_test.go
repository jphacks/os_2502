...
func testUploadImagesCollageResultsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(uploadImagesCollageResultPrimaryKeyColumns, uploadImagesCollageResultColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
