...
func testGroupsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(groupPrimaryKeyColumns, groupColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testGroupToManyCollageResults(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.CollageResults().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadCollageResults(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CollageResults); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.CollageResults = nil
	if err = a.L.LoadCollageResults(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CollageResults); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testGroupToManyGroupPartAssignments(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadGroupPartAssignments(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyUploadImages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadUploadImages(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyAddOpCollageResults(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddCollageResults(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.CollageResults[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.CollageResults[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.CollageResults().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testGroupToManyAddOpGroupPartAssignments(t *testing.T) {
	var err error
...
func testGroupToManyAddOpUploadImages(t *testing.T) {
	var err error
...
func testGroupToOneUserUsingOwnerUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOwnerUser(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGroupToOneSetOpUserUsingOwnerUser(t *testing.T) {
	var err error
...
var (
	groupDBTypes = map[string]string{`ID`: `char`, `OwnerUserID`: `char`, `Name`: `varchar`, `GroupType`: `enum('local_temporary','global_temporary','permanent')`, `Status`: `enum('recruiting','ready_check','countdown','photo_taking','completed','expired')`, `MaxMember`: `int`, `CurrentMemberCount`: `int`, `InvitationToken`: `char`, `FinalizedAt`: `timestamp`, `CountdownStartedAt`: `timestamp`, `ScheduledCaptureTime`: `timestamp`, `ExpiresAt`: `timestamp`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_            = bytes.MinRead
)
