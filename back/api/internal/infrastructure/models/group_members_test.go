...
func testGroupMembersExists(t *testing.T) {
	...
	e, err := GroupMemberExists(ctx, tx, o.GroupID, o.UserID)
	if err != nil {
...
func testGroupMembersFind(t *testing.T) {
	...
	groupMemberFound, err := FindGroupMember(ctx, tx, o.GroupID, o.UserID)
	if err != nil {
		t.Error(err)
	}
...
func testGroupMembersInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(groupMemberPrimaryKeyColumns, groupMemberColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testGroupMemberToOneSetOpGroupUsingGroup(t *testing.T) {
	...
	for i, x := range []*Group{&b, &c} {
		...
		if exists, err := GroupMemberExists(ctx, tx, a.GroupID, a.UserID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	groupMemberDBTypes = map[string]string{`ID`: `char`, `GroupID`: `char`, `UserID`: `char`, `IsOwner`: `tinyint`, `ReadyStatus`: `tinyint`, `ReadyAt`: `timestamp`, `JoinedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_                  = bytes.MinRead
)
...
func testGroupMembersUpsert(t *testing.T) {
	...
	if len(mySQLGroupMemberUniqueColumns) == 0 {
